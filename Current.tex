...
\begin{document}
	...
	An important feature of time Petri nets is the notion of urgency, that is, if a transition has been enabled then it must either be disabled (lose some tokens from its preset due to another transition firing) or be fired before the latest firing time lapses. It cannot be allowed to \emph{time-out}, waiting throughout the period of time is it capable of firing, unfired, and then be disabled just by having the Lft pass. 
	...
	Given a causal process $(CN, p)$ over $N$, where $CN = (B, E, G)$, and a (not necessarily valid) timing function $\tau : E \to \mathbb{R}^+$, we first define the auxilliary \textit{flow} function $f_{\tau} : E \to \mathbb{R}^+$ such that 
...
\end{example}
	...
	\begin{proof}
		By theorem \ref{thm35} proved in Aura and Lilius' article \cite{AL97}, we know that in order to ensure that this is a valid time process, we need only ensure three equalities hold as the time process $\gamma$ evolves. 
...
\begin{example}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.9\linewidth]{g34.png}
		\caption{$g_3$ and $g_4$ }
	\end{figure}
	\begin{figure}[h]
		\caption{$g_5$ and $g_6$}
		\includegraphics[width=0.9\linewidth]{g56.png}
	\end{figure}%
...
\end{remark}
	...
	What is interesting about these properties are the following results : 
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			\State $i \gets 1$
			\While {$i < n$}
			\State $a \gets \sigma - \gamma_i$
			\State $b \gets \sigma_{i+1} - \gamma_{i+1}$
			
			\If{$a\cdot b < 0$} 
			\Comment{$(a,0,i)$} %: Stability and co-operation desire opposite delays, so $d_i$ is zero}
		\State $\gamma \gets (a, 0, i) \gamma$
		
		\ElsIf{$|b| \leq |a|$}
		\Comment{$(a-b, b, i)$} %: Co-operation allows for completely correcting $i+1$ as well} 
	
	\State $\gamma \gets (a-b, b, i)\gamma$
	\Else
	\Comment{$(0, a, i)$}% : Stability desires a full delay, and co-operation allows no more}


\State $\gamma \gets (0, a, i) \gamma$
\EndIf

\State  $cost \gets cost + |a|$ \Comment{By chronology}
\State $i \gets i+1$
\EndWhile

\State $cost \gets cost + |\gamma_n - \sigma_n|$  \Comment $(0, |\sigma_n - \gamma_n|, n)$
...
\begin{theorem}\label{MixAlgoCorr} Algorithm \ref{MixWordAlgo} is correct, that is, its result \textsf(cost) $ = d_N(\gamma, \sigma)$
\end{theorem}

\begin{theorem}\label{MixAlign} Given a linear causal process of a time Petri Net $N$ and a linear observed trace $\sigma$, the word $\gamma_\theta \in \mathcal{L}(N)$ such that $\gamma_\theta = \argmin_{x \in \mathcal{L}(N)}{d_\theta(x, \sigma)}$ also has the property $\gamma_\theta = \argmin_{x \in \mathcal{L}(N)}{d_N(x, \sigma)}$. 
\end{theorem}
	...
	\begin{proof}
		We break up the proof of Theorem \ref{stablebest} into a few sections on order to prove optimality one property at a time. 
		
		\textsc{Chronology and Co-operation}
		
		\begin{lemma} Given two linear timed traces $\gamma, \sigma$ over the same causal process $(CN, p)$ and a sequence of moves $m'$ aligning $\gamma$ to $\sigma$, there exists a chronological sequence of moves $m$ that aligns $\gamma$ to $\sigma$ such that $cost(m) \leq cost(m')$.
		\end{lemma}
		...
		\begin{lemma} Given two linear timed traces $\gamma, \sigma$ over the same causal process $(CN, p)$ and a chronological sequence of moves $m'$ aligning $\gamma$ to $\sigma$, there exists a co-operative sequence of moves $m$ that aligns $\gamma$ to $\sigma$ such that $cost(m) \leq cost(m')$.
		\end{lemma}
		...
		Now, the proof of Theorem \ref{mixwordalgostable} is quite simply the following : 
		...
		By Lemma 7 we already know that there is only one stable sequence of moves aligning any two given linear timed traces. So, in order to prove the theorem, all we need to check is that each move calculated by the algorithm is stable. 
	...
	\subsection{Mixed Linear Models : Forwards and backwards}
