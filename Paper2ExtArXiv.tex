...
%
\institute{LMF, ENS Paris-Saclay, CNRS, Université Paris-Saclay, Inria,
Gif-sur-Yvette, France \email{neha.rino@ens-paris-saclay.fr}
\and
...
This paper follows a previous one, where we have set our problem of timed alignment. In the present paper, we solve the case where the metrics used to compare timed processes allow weighted mixed moves,
 i.e. an error on the timestamp of an event may or may not have propagated to its successors, and we provide linear time algorithms for a large class of such weighted mixed distances, both for distance computation and alignment on models with sequential causal processes. 
...
\section{Introduction} 
	...
	Process mining studies vast systems through their event logs, and seeks to extract meaningful ways to model the underlying patterns or processes that govern the behaviour of the system in order to better understand it, or predict future behaviour \cite{A16}.
	...
	This is given as a series of edits, usually insertions or deletions, that transform the observed trace into a process trace.  
	...
	In the untimed case, process models can be represented using a variety of formal objects, such as Petri nets. 
...
Distances over timed words have been explored before, such as in \cite{ABD18} and \cite{CJM14}, both of which have in common the notion of capturing the maximal difference between the timestamps attached to the same action label. 
Earlier, \cite{GHJ97} explored similar notions of absolute and relative errors but on traces whose event interdependencies are not linked to any model. 
	...
	For our purposes, we use time Petri nets, an extension of Petri nets equipped with the ability to express constraints on the duration of time between an action being enabled, and its actual occurrence. In particular, in this paper, we restrict ourselves time Petri nets with no branching points. 
...
Now, say we observed the trace $(u_1, \sigma_1) = (d, 4)(e, 6)(f, 6)$ that did not fit the model, and we wished to analyse how best to modify them to fit them back into the model. We note that the untimed part does match the model's specification, and in order to make the timestamps fit we can simply edit the timestamp of $d$ to give the following process trace $(v_1, \gamma_1) = (d, 4)(e, 6)(f, 6)$. 
	...
	We present linear time algorithms for both computing $d_N$ and solving the alignment problem in this setting, but once again only for linear timed words, and sequential process models.  
	...
	\begin{example}
		...
		Now consider aligning the timed traces $(4, 8, 11)$ and $(3, 7, 10)$. 
...
The cost of a sequence of moves is the sum of the costs of the moves. Armed with these definitions, upto scaling, all the distances we can build from these depend upon the ratio of stamp cost to delay cost, giving rise to a general class of weighted mixed distances. 
	...
	From this definition, we can recover the stamp only ($d_t$), delay only ($d_{\theta}$) and evenly mixed ($d_N$) distances defined in \cite{CR22} as natural special cases. 
		...
		$$d_{\infty}(\tau_1, \tau_2) = d_t(\tau_1, \tau_2) = \min \{cost(m) | {m \in Delay^*}, {m(\tau_1) = \tau_2}\}$$

	Hence forward for consistency in notation we will use $d_0, d_1, and d_{\infty}$ whenever referring to the same. 
	...
	\begin{example}\label{stamptoy}
		...
		Now we consider the evenly mixed setting. 
		Intuitively, when dealing with sequential models where the ripple effect of a delay is always to the right irrespective of the values of the timestamps, moves can be applied in any order, so we can assume a minimum cost run is chronological. 
...
Now, we look at a new way to represent timing functions. With delay edits, which constitute a new way of thinking about how a time-series can be transformed, comes a new perspective with which we can view timing functions over causal processes. 
	...
	\begin{example}\label{wordandflow}
		...
		Consider the word $w = (abc)(1, 5, 9)$. For this sequential timed trace, the flow function measuring its successive delays is $(1, 4, 4)$. 
	\end{example}
	...
	In order to compute the mixed distance between two timed traces, we wish to cull down the space of possible sequences of moves that transform the two words to each other to smaller, equivalent possibility spaces that still capture all the behaviours we're interested in, while being a little more well behaved. 
	...
	Chronology or order, the first property, represents the fact that moves can commute in a sequence. This holds for all $d_\alpha$, irrespective of $\alpha$. 
	...
	\subsubsection{Order}
	The first property captures the order in which moves in a sequence target the positions in a sequential trace. 
	It would be good to be able to focus our attention on sequences of moves that proceed in an orderly fashion from one end of the word to another, doing exactly one mixed move at each position. This is intuitively reasonable as doing multiple mixed moves at the same position can only worsen the cost if it at all changes it, and assuming that mixed moves commute on sequential traces, it would be all the same to arrange them in order of the position it takes effect at. This gives us the following two properties. 
	...
	\begin{example}
		...
		Given $\gamma = (1, 1, 2, 4, 5)$ and $\sigma = (1, 2, 2.5, 4.2, 5)$, an example of a cost 5.3 non-chronological sequence of evenly mixed moves aligning them is as shown below. 
	...
	\begin{example}The previous example (that was a cost 3.3 evenly mixed run, recall) can hence be further improved to become co-operative cost 1.7 run as follows : 
	...
	Once again, as the example suggests, one can always convert a an evenly mixed non-cooperative run into an equivalent cooperative one with cheaper cost, and in fact this holds for a large number $d_{\alpha}$ as well. 
	...
	Once again, as the example suggests, a stable chronological run (or reverse chronological run) is unique , but moreover, we claim that stability improves the cost of the sequence of moves.  
...
And lastly, the best $d_1$ alignment for the example in the diagram below is also $\gamma = (1,3,4)$, the sequence of moves being simply one stamp and one delay move at the start, $m = stamp(-1, 1)delay(-1, 1)$, and now with minimum cost $d_1(\sigma, \gamma) = 2 < \min \{d_t(\sigma, \gamma), d_\theta(\sigma, \gamma)\}$. 
\end{example}
...
We start with the proof of Lemma \ref{linescommute}. 

\begin{lemma*}For all $\alpha \in \mathbb{R}^+ \cup \{\infty\}$ there is a minimal cost sequence of $\alpha$ weighted mixed moves aligning any two sequential timed traces that is chronological/reverse chronological. 
\end{lemma*}
...
Now we prove Lemma \ref{linescoop}
\begin{lemma*}\label{linescoop} For all $\alpha \geq 2 \vee \alpha = 1$, there is a cooperative minimal cost chronological (or reverse chronological) sequence of $\alpha$ weighted mixed moves aligning any two sequential timed traces. 
\end{lemma*}
...
We proceed to the proof of Lemma \ref{crosscoop}. 

\begin{lemma*}	For every reverse chronological co-operative run $m'$ aligning $\gamma$ to $\sigma$, there is a reverse chronological, co-operative and cross co-operative run $m$ aligning $\gamma$ to $\sigma$ such that $$cost(m) \leq cost(m')$$
\end{lemma*}
...
\begin{lemma*}
	There is a stable, co-operative, reverse chronological minimal cost sequence of moves aligning any two sequential timed traces. 
\end{lemma*}
\begin{proof}
...
\subsection{Purely Timed Alignment over $d_1$ for Sequential Process Models} 
We can now look at the proof of Theorem \ref{MixAlign}. 

\begin{theorem*}
	Given a sequential process model $(CN, p)$ of a time Petri Net $N$ and a sequential observed trace $\sigma$, the trace $\gamma \in \mathcal{L}(N)$ such that $f_{\gamma}(i)= \underset{x \in [Eft(t_i), Lft(t_i)]} {\argmin}\hspace{0.5em} {|x - f_\sigma(i)|}$ also has the property $\gamma = \underset{x \in \mathcal{L}(N)}{\argmin} \hspace{0.5em}{d_N(x, \sigma)}$. 
\end{theorem*}
\begin{proof}
As $(CN, p)$ is sequential, we can consider the event set $E$ to be totally ordered by $G$, giving us the list $\{e_1, e_2, \dots e_n\}$ and their respective static interval constraints $\forall i \leq n : SI(p(e_i)) = [a_i, b_i]$. In this setting, clearly, $$f_{\gamma}(i)= \argmin_{x \in [a_i, b_i]} {|x - f_\sigma(i)|}$$
...
\end{proof}
\subsection{Computing $d_{\alpha}$ when $\alpha \geq 2$}
We take a brief interlude to show a simple proof for Lemma \ref{doubleisdelay}. 

\begin{lemma*} When $\alpha \geq 2$, for any sequence of $\alpha$ weighted mixed moves aligning any two sequential timed traces, there is a sequence of delay only moves that aligns the same two traces, and has at most equal cost. 
\end{lemma*}
\begin{proof}
