...
%        \todo[inline]{If we keep this section here, then slightly adapt the text.}
	...
	\end{theorem}
		...
		Now, we wish to minimize $|\gamma - \sigma|$, but it would be better to view this objective as a linear \todo{affine?} function. To this end we use a trick from \cite{BV2014} and use the following variable translation : $$\alpha, \beta \geq 0 : \alpha - \beta = \gamma - \sigma$$
	...
	\end{figure}
		...
		In the simpler case of sequential causal processes it might not seem obvious why aligning under $d_t$ should require such global analysis and quadratic time to perform. While we do not present a proof of hardness here, a modification of our running example might show how aligning under $d_t$ is unlikely to be doable by some sort of greedy or locally optimising algorithm. Consider a third net, $N''$ that has the same actions as $N'$ but the constraints on them are different. 
			...
			Let the observed trace be $\sigma = (reg, 0)(check, 0)(dec, 10)(pay,10)$.
			...
			A better choice would in fact be  $\gamma_{glob} = (reg, 5) (check, 10)(dec, 10)(pay, 10)$, as despite being very counter-intuitive at the start, it aligns better for the latter half of the trace, giving a lower total cost of $d_t(\sigma, \gamma_{glob}) = 15$. 
		...
		\end{example}
		%% \todo[inline]{I realise there was quite an important error in the example I kept here! I have fixed it now}
