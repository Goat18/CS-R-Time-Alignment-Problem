...
\begin{document}
	\title{Timed Alignments\\
		\thanks{Neha Rino was funded by the International Master's Scholarships Program IDEX of Université Paris-Saclay.}
                \thanks{This work was supported by French ANR (grant ANR-21-CE48-0003).}
	}
	...
	\author{\IEEEauthorblockN{Thomas Chatain}
		...
		\IEEEauthorblockA{\textit{IRIF, ENS Paris-Saclay,}\\
			\textit{CNRS, Université Paris-Saclay,} France\\
			\url{Neha.Rino@warwick.ac.uk}}
	}
	...
	\begin{abstract}
        ...
        In this paper, we formulate the general problem of timed alignment for process models represented as time Petri nets, and a purely timed version of the timed alignment problem that assumes only the timestamps of the trace need aligning rather than the actions. 
		Corresponding to one metric over timed traces, we present techniques to solve the purely timed alignment problem for extended free choice time Petri nets, and corresponding to another metric, we present a general method to compute purely timed alignments for bounded time Petri nets, and a more efficient direct method for a more restricted class of time Petri nets. 
		These techniques complement traditional untimed alignment methods to yield a preliminary approach for solving the general timed alignment problem. 
    \end{abstract}
    ...
    We often do not want the model to precisely generate all possible future system behaviours, but neither should it simply regurgitate the event log and accept no new system behaviours. 
    ...
    Time-aware process mining seeks to study both what the underlying processes governing system behaviour are, and what time constraints they obey \cite{RMAW13} \cite{CRHA20}  \cite{AS21}. 
    ...
    As time-aware process mining grows popular, new quality measures and conformance-checking techniques must be developed that are sensitive to temporal constraints, but so far in the study of alignments as a conformance-checking artefact, but as far as we know, the process model used is never time-aware. 
The problem of calculating alignments has been extensively studied \cite{Adriansyah2014AligningOA,BCC21} in this untimed setting
where one assumes that an event log is a list of words over a finite alphabet (the set of possible action labels), which can be compared using Levenshtein's edit distance or variants.
For time-aware conformance checking, one requires a notion of distance over timed words, enabling us to measure how close two timed process traces are. 
Distances over timed words have been explored before, such as in \cite{ABD18} and \cite{CIM14}, both of which have in common the notion of capturing the maximal difference between the timestamps attached to the same action label. 
Earlier, \cite{alldistHenz} explored similar notions of absolute and relative errors but on traces whose event interdependencies are not linked to any model (see section \ref{relwork} for a more detailed discussion about alternative notions of distance). 
%% \todo[inline]{Neha: I added another reference to work on timed distances here}
In this paper, we provide three distinct distance functions that can reflect information about multiple timestamp discrepancies between timed words, and using them, we pose and solve the alignment problem for time-aware processes.

    %\medskip%\subsection{Three Distances, and Algorithms to Align them}
    
 
Our contribution begins with formally setting the alignment problem and its purely timed version, which assumes that the trace to be aligned needs no edits to its actions, only its timestamps. We then propose three different distance functions over sequences of timestamps. 
We begin with our approaches for alignment under the simplest of the distance functions we defined over timed executions, $d_t$. 
For the wide class of process models that can be represented by bounded time Petri nets, we present an encoding of the purely timed alignment problem as a linear programming problem. 
This approach yields a polynomial upper bound on the worst-case running time of computing the purely timed alignment for most practical TPNs, and in practice linear programming problem solvers perform quite efficiently. We also take care to note that in the case of unbounded time Petri nets, most key properties such as reachability are undecidable, and as a result for sound and complete methods boundedness is a reasonable restriction. 
 % Still, the above method, both in theory and practice, takes at least $\mathcal{O}(n^3)$-time \todo[inline]{where does this bound come from?} to run, which is unnecessarily slow in the case of simpler process models. 
 Additionally, using an entirely different technique that we call cost-graphs, we present a direct, quadratic time $d_t$ alignment algorithm in the case of a structurally restricted class of models (sequential causal processes).
We also studied %\todo[inline]{rephrase?} 
 the purely timed alignment problem for a second distance function, $d_{\theta}$. 
%This distance function provides a new insight into the nature of time processes and their properties as it utilises the structure of the process model in its calculations. \todo[inline]{Thomas: I would remove the previous sentence from the introduction because the distance is not defined yet, so the reader cannot really understand.}
For all extended free choice time Petri nets, we present a straightforward algorithm for solving purely timed alignment in this case, whose time complexity is linear in the size of the causal process and the transition set of the model. 
This class of nets excludes precisely those time Petri nets that exhibit a behaviour known as confusion, which is often undesirable when modelling real-world systems. 

This paper is an extension of the ICPM 2022 conference paper \cite{CR22}, providing greater details into the proofs and techniques used to obtain our results. 
In particular, this includes a description of how to obtain the linear programming problem encoding that we use to show that the purely timed alignment for $d_t$ can be solved in polynomial time for bounded time Petri nets. 
We also provide a more detailed explanation of the direct, quadratic algorithm we provide for purely timed alignment under $d_t$ for sequential causal processes, discussing why the cost-graph technique we use is sound. 
We believe this method can be extended to a wider class of time Petri nets, and hence a more detailed look at this technique is valuable.
We also provide a section on related works about distances over timed words and time-series alignments, and discuss how and why our approach diverges from the literature.  
    
 We organise this paper as follows. We begin with defining timed words, time Petri nets, and the semantics of the execution of time Petri nets in section \ref{preliminaries}. Then, we proceed to section \ref{sec:alignmentpb}, where we formally set the alignment problem and its purely timed version, which assumes that the trace to be aligned needs no edits to its actions, only its timestamps. We then propose three different distance functions over sequences of timestamps. 
   In section \ref{lpp}, we present the encoding of the purely timed alignment problem under $d_t$, as a linear programming problem, and in section \ref{mcg} we present our direct, quadratic time algorithm in the case of a structurally restricted class of models (sequential causal processes).
    Then, in section \ref{4} we study purely timed alignment under $d_\theta$ and provide a linear time algorithm for computing the same for all extended free choice time Petri nets. We return to discussing how our method can complement traditional untimed alignment techniques to provide an approach for solving the general timed alignment problem and the limitations of the same in section \ref{align}. Section~\ref{implementation} reports on our implementation. We discuss related works in section \ref{relwork} and conclude in section \ref{conc}. 
   ...
   Since the main focus of this paper is a purely timed version of the alignment problem, we will often ignore the action labels of timed words, i.e., deleting the projection $\pi_1$ onto $\Sigma^*$, leaving just the timestamps, the projection $\pi_2$ onto $\mathbb{R}^{+*}$. 
    Especially when the underlying process model is sequential, we denote the time component of a timed trace as a sequence of non-decreasing timestamps, so the timed word $(a,1)(b,2)(c,3)$ is identified with its corresponding timestamp series $(1,2,3)$. 
    %% \todo[inline]{Neha : Inserted a reference to timestamp series notation}

    The timed process model we use here is a labelled time Petri net. 
    
    \begin{definition}[Labeled Time Petri Net] \label{tpn} A labeled \textit{time Petri net} (or TPN) is a tuple $N = (P, T, F, SI, \Sigma, \lambda,\linebreak M_0, M_f)$.
Here $P$, $T$ are disjoint sets of places and transitions respectively,  and $F \subseteq (P \times T) \cup (T \times P)$ is the flow relation that relates transitions to the places it takes tokens from and returns tokens to. 
The function $SI : T \rightarrow \mathbb{Q}^+ \times (\mathbb{Q}^+ \cup \{\infty\})$ is the static interval function which assigns to each transition an earliest and latest firing time, i.e.,  $SI(t) = (Eft(t), Lft(t))$ is a pair of positive rational numbers, the second of which might be infinity, such that $Eft$ stands for the earliest firing time, and $Lft$ for latest firing time. We use the function $\lambda : T \rightarrow \Sigma$ to label our transitions with actions from the action set $\Sigma$, and note that we do not consider silent transitions in this work (i.e. $\lambda$ is a total function). We let $ M_0 , M_f: P \to \mathbb{N}$ are the initial and final markings, that denote the initial and final configurations of tokens in the time Petri net. % of $N$. 
    \end{definition}
    
    Given a transition $t \in T$ we define the pre-set of $t$ as $\pre t = \{p \in P | (p, t) \in F\}$ and its post-set similarly is defined as $t \pre = \{p \in P | (t, p) \in F\}$, (the presets and post-sets of places are defined similarly). A transition on firing erases one token from each of its preplaces, and writes an additional token to each of its postplaces. 
    We say that a transition $t$ of a TPN is \textit{enabled} at a marking $M$ iff $\forall p \in \pre t :  M(p) > 0$, i.e., the transition is capable of consuming one token from each of its preplaces. 
    ...
    The first constraint encodes the purpose of the static interval function, while the second constraint encodes the notion of urgency, that is, if $t$ is enabled at marking $M$ and has clock value $Lft(t)$, it must fire, or another transition must fire at the same instant disabling $t$.  
...
$M''$ as in the untimed case (subtracting one token from each preplace, and adding one token to each postplace), and the clock function $I$ is updated to $I'$ by increasing clock values by $\theta$ for any transition that was enabled at both $M$ and $M'$, setting clock values to 0 for any transitions newly enabled by $M''$ that were not enabled at $M$, and leaving clock values undefined for any transitions that are not currently enabled. 
    ...
    \begin{definition}[Language of a TPN]
        A timed word $w = ((a_0, \tau_0), (a_1, \tau_1), \dots (a_n, \tau_n)) \in (\Sigma \times \mathbb{R}^+)^*$ is in the language $\mathcal{L}(N)$ of the labelled TPN $N$ if there is a valid execution of the model such that there exists a sequence of transitions $(t_0, t_1 \dots t_n) \in T^*$ satisfying $(\lambda(t_0),\lambda( t_1), \dots,\lambda (t_n)) = w $, such that when they fire at the times ($\tau_0, \tau_1 \dots \tau_n$), they transform the initial state into a final one, that is, there exists some clock function $I$ on $M_f$, $(M_0, \mathbf{0}) [t_0, t_1, \dots t_n\rangle (M_f, I)$, where each $t_i$ is fireable at $\tau_i$. 
    \end{definition}
    
Throughout we use the term \textit{valid execution} to distinguish it from faulty executions of the time Petri net. This is because we are studying alignments, and as a result studying how a non-fitting trace diverges in its ``execution'' from what is required by the time Petri net is our central concern. 

    \begin{example}\label{TPN}
        Consider the example (adapted from \cite{AS21}) of a TPN $N$ illustrated in Figure \ref{N}, modelling an airline refund office: 
      \begin{figure}
		...
		\adjustbox{scale = 0.6}{
            ...
            \begin{tikzcd}
                & {} & \Huge\bigcirc & {\fbox{\strut ec}} & \Huge\bigcirc & {} && {\fbox{\strut pay}} \\
                \Huge\bigcirc & {\fbox{\strut reg}} && {\fbox{\strut et}} && {\fbox{\strut dec}} & \Huge\bigcirc & {} & \Huge\bigcirc \\
                && \Huge\bigcirc & {\fbox{\strut{ct}}} & \Huge\bigcirc &&& {\fbox{\strut rej}}
		...
		\caption{An illustration of a time-aware process model for an airline refund office}
			\label{N}
	\end{figure} 
    ...
    We reuse classical definitions \cite{Winskel86}.
    \begin{definition}[Causal Net] \label{cn}
        A \textit{causal net} $CN = (B, E, G)$ is a finite, acyclic graph where 
     $B$ (conditions) and $E$ (events) comprise the vertices, $G$ denotes the edges (i.e., $G \subseteq (B \times E \cup E \times B)$) and $\forall b \in B : |b \pre| \leq 1 \wedge | \pre b| \leq 1$.
    \end{definition}
    
    This is meant to represent an unfolded run of the original Petri net, such that every time a place is revisited, it is copied afresh to ensure that the execution only ever moves forward. 
    Multiple elements of $B$ or $E$ map to the same element in $P$ or $T$ respectively, a notion that is expressed via the map $p$ defined below. Let $Min(CN)$ be the set of vertices of $CN$ that are minimal under the $G$ pre-ordering
    ...
    \begin{definition}[Homomorphism] \label{hom}  Let $N$ be a TPN with place set $P$ and transition set $T$, and ${CN = (B, E, G)}$ be a causal net. 
        A mapping $p : B \cup E \rightarrow P \cup T$ is a \textit{homomorphism} if $p(B) \subseteq P$,  $p(E) \subseteq T$ and $\forall e \in E$ the map $p$ when restricted to $\pre e$ is a bijection between $\pre e$ and $\pre p(e)$, and similarly, $p$ when restricted to $e \pre $ is a bijection between $e \pre $ and $p(e) \pre $, and lastly, $p$ when restricted to $Min(CN)$ is also a bijection between the sets of places $Min(CN)$ and $Supp(M_0)$, where $Supp(M_0)$ is the support of the marking function $M_0$.
    \end{definition}
    Such homomorphism naturally induces a labeling of the events: \(\lambda(e) = \lambda(p(e))\).
    In figures we use this labeling to identify the homomorphism.
...
%   \todo[inline]{Example of a CP here?}
    Using $p$, elements of $CN$ are identified with their corresponding \emph{originals} in the TPN, and as a result, any causal process corresponds uniquely to a run of its sequence of actions (an untimed run) on the untimed version of a given TPN (the underlying Petri net when the static interval function is ignored). 
    We will often label events in a causal process with the labels of their corresponding transitions in the original TPN. 
    ...
    When the underlying causal process is sequential, we can represent timing functions as a nondecreasing timestamp series, by projecting onto the timestamp component of any timed trace. 
...
%        \todo[inline]{If we keep this section here, then slightly adapt the text.}
    We take a moment to discuss the structurally restricted class of models, ones that generate causal processes whose graphical structure is that of a straight line. 
    ...
    \begin{example}\label{Branchingcp}
        Going back to the TPN $N$ discussed in example ~\ref{TPN}, in Figure ~\ref{run} we study the run of the same timed word  $$w = (reg, 1)(ec, 2)(ct, 3)(dec, 7)(rej,7)$$ 
		\begin{figure}
			...
			\adjustbox{scale = 0.6}{
				...
				\begin{tikzcd}
					& {} & \Huge\bigcirc & {\fbox{\strut \textcolor{blue}{ec}}} & \Huge\bigcirc & {} && {\fbox{\strut pay}} \\
					\Huge\bigcirc & {\fbox{\strut \textcolor{blue}{reg}}} && {\fbox{\strut et}} && {\fbox{\strut \textcolor{blue}{dec}}} & \Huge\bigcirc & {} & \Huge\bigcirc \\
					&& \Huge\bigcirc & {\fbox{\strut \textcolor{blue}{{ct}}}} & \Huge\bigcirc &&& {\fbox{\strut \textcolor{blue}{rej}}}
					\arrow["{[0, 1]}"{description, pos=0.2}, shift left=3, draw=white, from=2-2, to=1-2]
					\arrow[""{name=0, anchor=center, inner sep=0}, "{[2,2]}"{description, pos=0.1}, draw=white, from=2-4, to=1-4]
					\arrow["{[2,2]}"{description, pos=0.1}, draw=white, from=3-4, to=2-4]
					\arrow["{[2,4]}"{description, pos=0.1}, shift right=4, draw=white, from=2-6, to=1-6]
			...
			\caption{An illustration of the run of $w$ on $N$}
						\label{run}
		\end{figure} 

        We now construct the causal process of this execution (see Figure ~\ref{CP1}), and see that it is indeed branching, as the very first transition, $reg$, itself has two post-places, which violates the no branching condition $e \in E : |e \pre | \leq  1$. 
        \begin{figure}[!t]

			\centering
		\adjustbox{scale = 0.8}{
            \begin{tikzcd}
                \bigcirc & {\fbox{\strut reg}} & \bigcirc & {\fbox{\strut ec}} & \bigcirc & {\fbox{\strut rej}} & \bigcirc \\
                &\bigcirc& {\fbox{\strut ct}} & \bigcirc & {\fbox{\strut dec}} & \bigcirc
        ...
        \end{tikzcd}}
		\caption{An example of a branching causal process}
					\label{CP1}
	\end{figure}
    \end{example}
    
    \begin{example}\label{Sequentialcp}
       Let us now study a simplified version of the original process model, a new time Petri net $N'$ that abstracts all the checking actions into one transition called $check$ (see Figure ~\ref{SCPnet}).
\begin{figure}[h]

	\centering
	   \adjustbox{scale = 0.6}{
		\begin{tikzcd}
			& {{}} && {{}} && {{}} && {\fbox{ \strut \textcolor{blue}{pay}}} \\
			{\Huge{\bigcirc}} & {\fbox{ \strut \textcolor{blue}{reg}}} & {\Huge{\bigcirc}} & {\fbox{ \strut \textcolor{blue}{check}}} & {\Huge{\bigcirc}} & {\fbox{ \strut \textcolor{blue}{dec}}} & {\Huge{\bigcirc}} & {{}} & {\Huge{\bigcirc}} \\
			&&&&&&& {\fbox{\strut rej}}
			\arrow[from=2-1, to=2-2]
			\arrow[from=2-2, to=2-3]
			\arrow[from=2-3, to=2-4]
			...
			\arrow[curve={height=12pt}, from=2-7, to=3-8]
			\arrow[curve={height=12pt}, from=3-8, to=2-9]
			\arrow["{[0,1]}"{description, pos=0.2}, draw=none, from=2-2, to=1-2]
			\arrow["{[2,2]}"{description, pos=0.2}, draw=none, from=2-4, to=1-4]
			\arrow["{[0,1]}"{description, pos=0.2}, draw=none, from=1-8, to=2-8]
			\arrow["{[0,0]}"{description, pos=0.1}, draw=none, from=3-8, to=2-8]
			\arrow["{[2,4]}"{description, pos=0.3}, draw=none, from=2-6, to=1-6]
		\end{tikzcd}
			}
			\caption{An illustration of a simpler time Petri net, $N'$}
				\label{SCPnet}
			\end{figure}

	    Here, we see that  all the branching and joining happens at places rather than transitions, so its executions all have sequential causal processes, such the causal process in figure ~\ref{CP2} for the untimed word $w = reg \cdot check \cdot dec \cdot pay$. \\
		...
		\begin{figure}[h]
            ...
            \adjustbox{scale = 0.6}{
                \begin{tikzcd}
                    \Huge\bigcirc & {\fbox{\strut reg}} & \Huge\bigcirc & {\fbox{\strut check}} & \Huge\bigcirc & {\fbox{\strut dec}} & \Huge\bigcirc & {\fbox{\strut pay}} & \Huge\bigcirc 
                    \arrow[from=1-1, to=1-2]
                    \arrow[from=1-2, to=1-3]
                    \arrow[from=1-3, to=1-4]
                    \arrow[from=1-4, to=1-5]
			...
			\caption{An example of a sequential causal process}
						\label{CP2}
		\end{figure}   
        This causal process is unrolled from the Petri net, exactly the way runs of finite state automata are simple paths over the graph of the automaton. 
        We throughout assume that wherever needed, such a causal process can be obtained via breadth-first search. 
    \end{example}
	...
	\begin{example}
		Consider the process model $N$ from Figure ~\ref{N} and an observed trace $(reg, 1) \cdot (ec, 2) \cdot (ec, 2) \cdot (ct, 3) \cdot (dec, 8) \cdot (rej, 8)$ . 
		Clearly, one shouldn't examine casually twice so one of the $ec$'s is extraneous, and also, the $dec$ event fires too late, it should have waited at most four time units after the firing of event $ct$. 
	...
	This example witnesses the types of domain-specific tradeoffs one may have to consider when devising an approach for the general timed alignment problem, and we discuss this issue at greater length in Section ~\ref{align}. 
	In order to avoid having to make such choices, we follow the principle used in \cite{CIM14}, wherein the edit distance over a timed word is decoupled into first aligning its actions, and once those are assumed to be well aligned, to proceed with aligning timestamps. 
	Hence, in this paper, we restrict our attention to the case where the untimed part of $\sigma$ does match the model, that is to say, if $\pi_1(\sigma)$ has a valid causal process $(CN, p)$ over $N$. 
	Clearly this case of the problem needs to be solved if the general timed alignment problem is to be solved, and we argue that this case is interesting and complex in its own right. 
	...
	We shall return to the general timed alignment problem in section ~\ref{align}, and briefly discuss some ways our methods can be combined with existing (untimed) alignment techniques to provide initial approaches to the general timed alignment problem. 
	These initial approaches have clear limitations, but they provide a first step towards addressing the general timed alignment problem. 
	...
	\begin{example}
		Let us go back to the TPN $N$ in Figure ~\ref{N}. 
		...
		A way to arrive at this conclusion is by noticing that on trying to execute $\sigma$'s firing sequence only the guard for $ct$ fails (See figure ~\ref{stamperror}). 
		\begin{figure}[h]
			...
			\adjustbox{scale = 0.6}{
				...
				\begin{tikzcd}
					...
					\arrow["{[0, 1]}"{description, pos=0.2}, shift left=3, draw=white, from=2-2, to=1-2]
					\arrow[""{name=0, anchor=center, inner sep=0}, "{[2,2]}"{description, pos=0.1}, draw=white, from=2-4, to=1-4]
					\arrow["{\textcolor{red}{[2,2]}}"{description, pos=0.1}, draw=white, from=3-4, to=2-4]
					\arrow["{[2,4]}"{description, pos=0.1}, shift right=4, draw=white, from=2-6, to=1-6]
					\arrow["{[0,1]}"{description, pos=0.1}, draw=white, from=1-8, to=2-8]
					\arrow["{[0,0]}"{description, pos=0.1}, draw=white, from=3-8, to=2-8]
					\arrow[from=2-1, to=2-2]
			...
			\caption{An illustration of a faulty execution on $N$}
						\label{stamperror}
		\end{figure} 


		If $ct$ were shifted to fire at 3 instead, the whole run would execute without a hitch. (Note that we allow timestamps to be out of order when representing timed words, so long as the overall partial order imposed by the process model is preserved).
		...
		A closest valid timing function would try to preserve the positions of $reg$ and  $ec$ but the moment it tries to fire $ct$, $\sigma$ runs early. 
		As seen in Figure ~\ref{stamperror}, the only guard that was failed was that of $ct$, and if one only locally edits the trace, it changes the values of the clocks at all the subsequent guards, which implicitly assumes that all the future events compensate by taking less time to complete, in order to stay on track overall.  
		By pushing $ct$ forward, it should take longer for the decision to be made and the rejection to happen, as those events depended on the checking of the ticket being done first. 
		This causes a cascading chain of errors where if $ct$ is moved forward to fire at 3, to preserve the relative relationships between $ct$ and its successors, we get the trace $\gamma' = (reg, 1)\cdot  (ct, 3)  \cdot (ec, 2) \cdot (dec, 7) \cdot (rej, 7)$, and in this sense, $\gamma'$ is closer to $\sigma$ as it preserves more relative gaps. 
		As we just knocked one event a little later by 1 unit, we can view this as a cost 1 edit too.  
	...
	Inspired by the above example, we consider two types of edit moves for timed executions. 
	Both of these moves make clear conceptual sense, and result in different types of insight about the root cause of faults in ill-fitting traces. 
	...
	\begin{definition}[Delay Move]
		Given a timing function $\gamma : E \to \mathbb{R}^+$, we define a delay move applied to it as follows : 
	...
	The cost of a move is the magnitude $|x|$ in the above definitions, and the cost of a sequence of moves is the sum of the costs of the moves. 
	Let the set of all stamp moves on a causal process be called $Stamp$, and the set of all delay moves be called $Delay$. 
	Armed with these definitions, three natural notions of distance can be constructed, which aim to minimise the cost of editing traces using one or both of these types of moves. 
...
We return to the simplified net $N'$ that we discussed first in Example ~\ref{Sequentialcp}. 
\begin{figure}[h]
	\centering
	   \adjustbox{scale = 0.6}{
		\begin{tikzcd}
		...
		\end{tikzcd}
			...
			\end{figure}
                       ...
                       % \todo[inline]{make sure that the notation for timing functions (without action names) is well introduced.}
		Now for this TPN, let the observed trace's timing function ${\sigma = (3, 4, 8, 9) \not \in \mathcal{L}(N')}$. 
		
		The best $d_t$ alignment for the example in the diagram below is $\gamma = (1,3,7, 8)$ with minimum cost $d_t(\sigma, \gamma) = 5$. 
		
		The best $d_\theta$ alignment for the example in the diagram
                below is also $\gamma = (1,3,7, 8)$, but this time with minimum
                cost $d_\theta(\sigma, \gamma) = 3$, evidenced by the move
                sequence $(delay(-2, 1)delay(+1, 2))$.
	...
	\end{example}
	  ...
	  \begin{itemize}
          ...
          %% \item $d_N(\tau_1, \tau_2) \neq \min{(  d_t(\tau_1, \tau_2), d_\theta(\tau_1, \tau_2))}$
	  ...
	  %% The third is proven by the existence of example \ref{stamptoy}. 
	...
	In this section, we focus on studying the distance function $d_t$. Hence, we present a useful lemma regarding an alternative equivalent definition of $d_t$, an upper bound on the complexity of solving the purely timed alignment under $d_t$ for bounded TPNs, and a faster algorithm for solving it for a restricted subclass of TPNs. 
	\begin{lemma}[Stamp only distance : $d_t$]\label{manhattan}
		
		
		$d_t$ as defined above is equivalent to the \textit{Manhattan distance} or \textit{taxicab distance} between two points of $\mathbb{R}^n$, where $|E| = n$, say $\mathbf{t} = (t_1, t_2, \dots t_n) $ and $\mathbf{s} = (s_1, s_2, \dots s_n)$, defined as $$d_{\textrm{taxicab}}(\mathbf{t}, \mathbf{s}) = \sum_{i = 1}^{n} |t_i - s_i|$$
	\end{lemma}
		\begin{proof}
		Given a causal process $CN = (B, E, G)$ and two timing functions (not necessarily constituting valid timings) $\tau_1, \tau_2$, let $$d_{\textrm{taxicab}}(\tau_1, \tau_2) = \sum_{e \in E} |\tau_1(e) - \tau_2(e)|$$
		
		It can be deduced that $d_t \leq d_{\textrm{taxicab}}$ as the sequence of stamp only moves consisting of the corresponding $\tau_1(e) - \tau_2(e)$ stamp move at each event edits the first word into the second word. 
		
		On the other hand, the least cost any stamp only path that edits the first trace into the second requires is $d_{\textrm{taxicab}}$, due to the triangle inequality, and because every individual stamp move results in a word at exactly the same taxicab distance from its predecessor as the cost of said stamp move. 
		Hence, we can conclude that $$d_t(\mathbf{t}, \mathbf{s}) = \sum_{i = 1}^{n} |t_i - s_i|.$$
	\end{proof}

	
	
	This lemma also allows us to encode the purely timed alignment problem for $d_t$ as a linear programming problem, as we see below. 
	...
	Using these firing domains, states are grouped into state classes,which are sets of states with clock functions in the same firing domain, and for bounded time Petri nets there are finitely many such state classes. 
	...
	\begin{theorem}\label{convexset}
		Given a bounded TPN $N$, an observed trace $\sigma$, and its causal process $CN = (B, E, G)$, the purely timed alignment problem seeks the vector $(\gamma_1, \gamma_2, \dots \gamma_{|E|})$ that minimizes the quantity $\sum_{e \in E} |\gamma_e - \sigma_e|$ over the set $\gamma \in \mathcal{L}(N)$. 
		
		This can be cast as a linear programming problem, i.e, we can effectively construct the convex set $\mathcal{C}$ such that the following optimization problem is equivalent to solving the purely timed alignment problem. $$\underset{\alpha - \beta \in \mathcal{C}, \alpha, \beta \geq 0}{\mathsf{minimize}} \hspace{0.5em} \alpha + \beta  $$
	...
	\end{theorem}
		...
		As $\gamma \in \mathcal{L}(N)$  this means it obeys the set of inequalities corresponding to the state class containing the final marking. 
		Now, the above constraint is convex, so $\gamma$ varies over a convex set. 
		As $\sigma$ is a constant vector, translation by it will keep the space convex, so $\gamma - \sigma \in \mathcal{C}$ for some convex set $\mathcal{C}$. This convex set $\mathcal{C}$ is the one we will utilise as the domain of our convex optimization. 
		
		Now, we wish to minimize $|\gamma - \sigma|$, but it would be better to view this objective as an affine 
		function. To this end we use a trick from \cite{BV2014} and use the following variable translation : $$\alpha, \beta \geq 0 : \alpha - \beta = \gamma - \sigma$$
		
		As defined above, the sum of $\alpha$ and $\beta$ is minimal when they are equal to the absolute values of the nonnegative and negative components of the vector $\gamma - \sigma$, respectively, i.e. when their sum is $|\gamma - \sigma|$. 
		Our alignment problem can now be equivalently cast as $$\underset{\alpha - \beta \in \mathcal{C}, \alpha, \beta \geq 0}{\mathsf{minimize}} \hspace{0.5em} \alpha + \beta  $$

hence, we have proved that the $d_t$ alignment problem for bounded TPNs can be encoded as a linear programming problem.		\end{proof}

	
	%% \begin{remark}
		
		By the above theorem we see that the alignment problem for $d_t$ is easily cast as a linear progamming problem, and can be solved by any of the class of ellipsoid or simplex methods for the same.
		We take care to note that this is not an encoding into an Integer Linear Programming problem (ILP), which is much more complex to solve. 
		 Hence we have a polynomial time upper bound on the complexity of this problem. 
		...
		This bound is not necessarily tight, but either improving it or providing a matching lower bound are left for future work. 
		While the general case seems to be hard to solve efficiently, there is a subclass of process models for which the problem seems more tractable, and moreover, has a quadratic time solution. 
		

		\subsection{A Direct Algorithm for Sequential Models}\label{mcg}
		In this subsection, we focus on solving the purely timed alignment problem for $d_t$ when the time Petri net in question branches only at places, not transitions. 
		This is a significant restriction on the model, but the upshot is that it allows us to use a new technique to solve the purely timed alignment problem in quadratic time. 
		The central object of study here is the cost-graph, an effectively computable and storable function that tells us the optimal cost of partially aligning a trace to the model, as a function of the last timestamp it chooses. 
		In this subsection we first define and discuss cost-graphs, then see a method for effectively computing them, and computing the overall optimal alignment in the end. 

		\subsubsection{Cost-graph functions ($g_i$)}
		...
		\begin{example}\label{stampcomp}
			...
			As these are all piecewise affine graphs, summing is simply incrementing the slopes of all the segments of the black-orange curve by one if they're to the right of $\sigma_i$, and by minus one if they're to the left, and translating the height up as needed. 
	...
	\begin{lemma}\label{minpwc}
		For any convex piecewise affine function $g$, the following function $g'$ is also convex and piecewise affine. 
	...
	\begin{proof}
		...
		So we see that for any convex piece-wise affine function $g$, $$\min_{d-d' \in [a,b]} g(d') = \begin{cases}
			g(d-a) & l + a \leq d < m + a \\
			g(m) & m+a \leq d \leq m' + b \\
			g(d+b) & m' + b < d \leq h + b   \\
		\end{cases}$$
		
		Hence $g'$ is also convex and piecewise affine, and has at most one more affine segment than $g$. 
	\end{proof}
	...
	We may now prove Theorem ~\ref{StampAlgoCorr}. 
		...
		To this end, we first notice that the family of functions $g_i(d)$ are convex and piecewise affine. 
		This can be seen through induction : 
		
		Firstly $g_1(d) = |d - \sigma_1|$ so the base case is covered. 
		
		Now suppose $g_{i-1}$ is known to be convex and piecewise affine for some $i \geq 2$. 
		By Lemma ~\ref{minpwc} we see that $\min_{d-d' \in [a_i,b_i]} g_{i-1}(d')$  is also convex and piecewise affine, and so is $|d - \sigma_i|$ and both these properties are preserved by addition, so $g_i(d)$ must also be convex and piecewise affine. 
	...
	\end{proof}
		...
		\begin{example}\label{altdthard}
			In the simpler case of sequential causal processes it might not seem obvious why aligning under $d_t$ should require such global analysis and quadratic time to perform. 
			While we do not present a proof of hardness here, a modification of our running example might show how aligning under $d_t$ is unlikely to be doable by some sort of greedy or locally optimising algorithm. 
			Consider $N'$ as defined before :  
				...
				Let $\sigma = (reg, 1)(check, 2)(dec, 4)(pay,6)$ be an observed trace.
				There as a number of optimal $d_t$ alignments for this trace such as $\gamma_1 = (reg, 0)(check, 2)(dec, 4)(pay,5)$ or 	$\gamma_2 = (reg, 1)(check, 3)(dec, 5)(pay,6)$, both of which have net cost $2$ but look quite different. 
				It is this tendency for edits early on in the trace to force later parts to fit closer to the observation under $d_t$ that makes aligning under it more subtle than with $d_{\theta}$.  		
	
			\end{example}
		\begin{example}\label{dthard}
		In fact, if we further change the constraints in our  net, we can see that not only can some of the optimal traces can look unintuitive, but the intuitive traces can be strictly suboptimal : 

			\adjustbox{scale = 0.6}{
				\begin{tikzcd}
					& {} && {} && {} &&{} \\
					\Huge\bigcirc & {\fbox{\strut reg}} & \Huge\bigcirc & {\fbox{\strut check}} & \Huge\bigcirc & {\fbox{\strut dec}} & \Huge\bigcirc & {\fbox{\strut pay}} & \Huge\bigcirc 
					\arrow["{[0, 5]}"{description, pos=0.2}, draw=white, from=2-2, to=1-2]
					...
					\arrow["{[0,5]}"{description, pos=0.2}, draw=white, from=2-4, to=1-4]
				\end{tikzcd}
			}\\
			
			Consider $\sigma = (reg, 0)(check, 0)(dec, 10)(pay,10)$.
			Naively starting from the beginning, one might be tempted to construct the aligning trace $\gamma_{loc} = (reg, 0) (check, 0)(dec, 0)(pay, 0)$, but, the obvious best choices at the start of the trace incur heavy costs towards the second half, giving a total cost of $d_t(\sigma, \gamma_{loc}) = 20$. 
			
			However, $\gamma_{glob} = (reg, 5) (check, 10)(dec, 10)(pay, 10)$ is a strictly better choice, as despite being very counter-intuitive at the start, it aligns better for the latter half of the trace, giving a lower total cost of $d_t(\sigma, \gamma_{glob}) = 15$. 
		...
		In this section, we focus on the delay-only setting, studying the purely timed alignment problem in depth for $d_{\theta}$. 
		To this end, we begin by setting up some relevant notation, and prove some basic lemmas, before proceeding to the main result, an efficient and correct algorithm for solving the purely timed alignment problem for $d_{\theta}$ over any extended free choice time Petri net. 
...
Note that for extended free choice time petri nets, in order to check for the validity of a timing function, checking all of $C_e$ as before is no longer necessary. 
	...
	\end{proof}
		...
		\end{remark}
                ...
                We have implemented the stamp only alignment algorithm in Python, available at \url{https://github.com/NehaRino/TimedAlignments}. 
				This algorithm is quadratic in time complexity, and was seen to run well in practice when tested on an Apple M1 chip with 8 cores, running at 3.2 GHz, 8GB RAM machine, as seen below. 
...
%\todo[inline]{Thomas: Neha, could you insert the configuration on which you ran the experiments, (Processor, frequency, RAM)?}

		\section{General Timed Alignment Problem: Approaches and Limitations} \label{align}
		...
		This approach is quite naive, as it has a tendency to exaggerate the importance of action label errors. 
		Implicitly, it is best for minimising the pair consisting of action edit costs and time edit costs, in lexicographic order, and as such would prefer even small improvements to the action edit cost at the price of a much greater timestamp cost. 
		...
		\begin{example}
			...
			\adjustbox{scale=0.8}{
				\begin{tikzcd}
					\eye & {\fbox{\strut a}} & \bigcirc & {\fbox{\strut a}} & \bigcirc & {\fbox{\strut a}} &\bigcirc \\
					& {\fbox{\strut b}} & \bigcirc & {\fbox{\strut a}} & \bigcirc & {\fbox{\strut a}} \\
					\arrow[from=1-1, to=1-2]
		...
		The issue here is the tradeoff between timestamp edits and action label edits. 
		A better approach would assign one cost $c_A$ to action edits and another $c_T$ to time edits, and seek to minimise their sum. 
		A potential fix to our previous idea would be to essentially retain the above approach, but instead of just doing it for the best untimed alignment, take a selection of good untimed alignments (up to some allowable threshold), and try to align timestamps for each untimed candidate, and then minimise the total cost over this set. This alleviates some of the bias towards preserving action labels by allowing for small action deviations in case the timestamp alignment proves particularly cost efficient, but this is still a naive approach to the wider problem, and we hope to find better solutions in the future. 
		
		\section{Related Work}\label{relwork}

		There are a number of existing definitions of distance functions over timed words, such as the six topologically equivalent definitions listed in \cite{alldistHenz}. 
		Two of these distances are notably very similar to our stamp and delay based distances, and much like in our setting, the distance between two words is only defined when the action sequences match up exactly. 
		In \cite{CIM14}, a notion of edit distance for timed words is introduced; similarly to our approach for solving the general timed alignment problem, given two timed words, it first tries to edit the untimed parts to match optimally, and then proceeds to edit their timestamps using stamp-like edits, yielding not a real value as the distance but a pair of values, the first component being the action sequence's edit distance and the latter being the stamp distance on the transformed timing sequences. 
		In \cite{ABD18}, the distance function finds the maximal timing deviation between any two events that correspond to the same action, and is closely related to the Hausdorff distance between point-sets. 
		The latter two allow for differences in the number and placement of actions in the word, while the first work does not. 
		An important way we deviate from all of these distances, is the way $d_{\theta}$ reflects the causal dependencies of the model. 
		As we operate within the domain of process mining, an important goal in the field is to leverage the insight that process models provide about the systems they represent. 
		Whether we are measuring the fitness of the model, trying to repair the model, or trying to determine how to best replay a faulty trace on a process model we trust, using the fact that the causal order imposed by the model would make errors cascade forward allows us to better study the model, and all of its causal implications, when computing the alignment. 
		In case when aligning a certain trace, $d_t$ comes out to be large but $d_{\theta}$ returns a much smaller number, we know that the process model predicts that the deviation that caused the trace to fail can be modelled as localised to fewer events, which caused knock-on effects later on, simplifying the isolation and repair of faults. 
		Hence, in this domain, we believe that making our distance dependent on the process model enriches the information we receive via alignment. 

		Given that this paper mainly focuses on the purely timed aspect of alignment, we take a moment to note that alignment between timed traces has been well studied in the time-series community. 
A popular method used for discrete time series is Dynamic Time Warping \cite{DTW}, which is a notion of edit distance over finite (possibly unequal length) sequences of real numbers, where the cost is minimised over a many-one matching between the timed sequences. 
Another is the Geometric Edit Distance \cite{GEdit}, which is similar to Dynamic Time Warping but it assumes the real sequences are matched using a one-one function, and if any points are missed out, each missing point incurs a constant additional cost. 
As actions do not have a nontrivial notion of distance between them, the best way of applying these to our setting is to consider timestamp sequences indexed by their position in the word. 
In this case, Dynamic Time Warping seems to compute distances between traces assuming that one word is a stutter of another, while Geometric Edit Distance will allow for deleting some events (possibly ones whose actions do not match), and both of these alternatives seem to yield distance measures whose meaning in the model are unclear. 
These distances also have more continuous counterparts such as the Skorohod distance \cite{skorokhod1956limit} and the Fr\'echet distance \cite{frechet1906quelques}, but we restrict ourselves to the case of discrete timed event sequences. 

		
		\section{Perspectives and Conclusion}\label{conc}
		
		In this paper, we pose the alignment problem for timed processes, including the general problem, and a variant known as the purely timed alignment problem, which assumes the actions of the trace to be aligned already fit the model. 
		We then devised three metrics for studying alignments for timestamp sequences, and solved the purely timed alignment problem for the first two metrics proposed. 
		For the first of the two metrics, when aligning traces to bounded time Petri nets, we provide a general, polynomial time approach using a linear programming problem encoding, which in practice can be solved efficiently. 
		For the same metric, over a structurally restriced subclass of time Petri nets that yield sequential causal processes, we have a direct, quadratic time algorithm using a new method we developed, the cost-graph method. 
		For the second metric, over extended free choice time Petri nets, we provide a simple, linear time method for computing purely timed alignments. 
		As far as we know, this is the first step in conformance checking for time-aware process mining, and much further work can be inspired from this point. 
		The alignment problem for the third metric $d_N$ is a first future direction. 
		Secondly, for the first metric studied here, $d_t$, the class of models for which the alignment problem was solved efficiently is structurally restricted (those with sequential causal processes) and it would be interesting to see how to generalise our cost-graph method to wider classes of time Petri nets. 
		Also, while the definitions of the metrics and problems presented here do readily adapt to other types of timed process models such as time transition/place Petri nets or timed-arc Petri nets, the methods for solving the problems may not.
		Thirdly, further investigation in the general timed alignment problem is necessary, as our proposed approaches here has clear limitations, and at the moment simply complements traditional untimed alignment rather than actually generalising to account for tradeoffs between action edits and timestamp edits. 
		Lastly, there are a number of other conformance artefacts that can be defined and studied in the timed setting, such as anti-alignments \cite{CBC21}, and one can better develop all such conformance checking methods to account for timed process models.
		%%mention g-algo specifically?
		\bibliographystyle{IEEEtran}
		\bibliography{icpmextended}
