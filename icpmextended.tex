...
This paper is an extension of the ICPM 2022 conference paper \cite{CR22}, providing greater details into the proofs and techniques used to obtain our results. In particular, this includes a description of how to obtain the linear programming problem encoding that we use to show that the purely timed alignment for $d_t$ can be solved in polynomial time for bounded time Petri nets. We also provide a more detailed explanation of the direct, quadratic algorithm we provide for purely timed alignment under $d_t$ for sequential causal processes, discussing why the cost-graph technique we use is sound. We believe this method can be extended to a wider class of time Petri nets, and hence a more detailed look at this technique is valuable. 
 ...
 We organise this paper as follows. We begin with defining timed words, time Petri nets, and the semantics of the execution of time Petri nets in section \ref{preliminaries}. Then, we proceed to section \ref{sec:alignmentpb}, where we formally set the alignment problem and its purely timed version, which assumes that the trace to be aligned needs no edits to its actions, only its timestamps. We then propose three different distance functions over sequences of timestamps. 
   In section \ref{lpp}, we present the encoding of the purely timed alignment problem under $d_t$, as a linear programming problem, and in section \ref{mcg} we present our direct, quadratic time algorithm in the case of a structurally restricted class of models (sequential causal processes).
	...
	Especially when the underlying process model is sequential, we denote the time component of a timed trace as a sequence of non-decreasing timestamps, so $(a,1)(b,2)(c,3)$ corresponds to the timestamp series $(1,2,3)$. 
	...
	\begin{definition}[Labeled Time Petri Net] \label{tpn} A labeled \textit{time Petri net} (or TPN) is a tuple $N = (P, T, F, SI, \Sigma, \lambda, M_0, M_f)$, where $P, T$ are disjoint sets of places and transitions respectively,  $F \subseteq (P \times T) \cup (T \times P)$ is the flow relation,  $SI : T \rightarrow \mathbb{Q}^+ \times (\mathbb{Q}^+ \cup \{\infty\})$ is the static interval function, where $SI(t) = (Eft(t), Lft(t))$ is a pair of positive rational numbers, the second of which might be infinity, such that $Eft$ stands for earliest firing time, and $Lft$ for latest firing time, $\lambda : T \rightarrow \Sigma$ is the labeling function, labeling transitions with actions from the action set $\Sigma$, and  $ M_0 , M_f: P \to \mathbb{N}$ are the initial and final markings. % of $N$. 
	\end{definition}
	...
	In this work, we do not consider silent transitions. 
