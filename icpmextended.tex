...
\begin{document}
    ...
    The notion of distance used over these words used here is usually either Hamming distance or Levenshtein's edit distance. 
...
This approach yields a polynomial upper bound on the worst-case running time of computing the purely timed alignment for most practical TPNs, and in practice linear programming problem solvers perform quite efficiently. We also take care to note that in the case of unbounded time Petri nets, most key properties such as reachability are undecidable, and as a result for sound and complete methods boundedness is a reasonable restriction. 
  Still, the above method, both in theory and practice, takes at least $\mathcal{O}(n^3)$-time to run, which is unnecessarily slow in the case of simpler process models. Hence, using an entirely different technique that we call cost-graphs, we present a direct, quadratic time $d_t$ alignment algorithm in the case of a structurally restricted class of models (sequential causal processes).
Now, for our contributions towards solving the purely timed alignment problem for a second distance function, $d_{\theta}$. This distance function provides a new insight into the nature of time processes and their properties as it utilises the structure of the process model in its calculations. 
For all extended free choice time Petri nets, we present a straightforward algorithm for solving purely timed alignment in this case, whose time complexity is linear in the size of the causal process and the transition set of the model. This class of nets excludes those precisely those time Petri nets that exhibit a behaviour known as confusion, which is often undesirable when modelling real-world systems. 
