...
In this paper we provide distinct distance functions that can reflect information about multiple timestamp discrepancies between timed words, and using them, we pose and solve the alignment problem for time-aware processes.
	...
	Especially when the underlying process model is linear, we denote the time component of a timed trace as a linear sequence of non-decreasing timestamps, so $(a,1)(b,2)(c,3)$ corresponds to the timestamp series $(1,2,3)$. 
...
%	\todo[inline]{Example of a CP here?}
	...
	When the underlying causal process is linear, we can represent timing functions as a nondecreasing timestamp series, by projecting onto the timestamp component of any timed trace. 
