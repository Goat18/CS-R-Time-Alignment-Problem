...
There is also earlier work \cite{GHJ97} that explores similar notions of absolute and relative errors but on traces whose event interdependencies are not linked to any model. 
%% \todo[inline]{Neha : I added another reference to a work on timed distances here}
In this paper we provide distinct distance functions that can reflect information about multiple timestamp discrepancies between timed words, and using them, we pose and solve the alignment problem for time-aware processes.
...
%        \todo[inline]{If we keep this section here, then slightly adapt the text.}
	...
	%	  Neha : I think they're very nice! Maybe we can explicitly keep the property the preceding example proves too here? I've added it in the third spot, but I'm not sure it needs repeating, do feel free to delete it if you'd like.\\
      %            Thomas: There is a problem with the quantifier: all the inequalities are below a forall. The one you added would need an exists. So I delete it.}
        \begin{lemma}
	...
	%% \begin{remark}
		...
		\begin{example}\label{stampcomp}
			...
			We continue similarly : The next black graph is a translation of the previous blue one, with an orange line segment or the same length as the next static interval constraint inserted at the lowest point, and the next green graph is $|x-\sigma_i|$ in the new domain, and summing these two gives the next blue graph. 
                       % \todo[inline]{Thomas: I changed linear to affine in the next paragraphs}
