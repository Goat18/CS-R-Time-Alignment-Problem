...
As for process models, there are ways to use existing process mining notation to denote time constraints. BPMN 2.0 comes equipped with \emph{timer events}. 
    ...
    In particular, in this paper, we restrict ourselves to time Petri nets with \color{blue}no branching transitions. \color{black}
...
%   \subsection{Timed Alignments}
    ...
    \begin{example} Consider a model of composing and sending an email, where the intervals signify allowed durations between the current event and its immediate predecessor, depicted below with input and output places marked with an $i$ and an $o$: 
        ...
        \color{red}One process trace is $(\textrm{save draft}, 3)(\textrm{dispatch}, 5)(\textrm{sent}, 10)$, which depicts drafting the message in 3 units and then dispatching it in 2 units, and having it send successfully.
...
\color{red}It includes a more in depth discussion of the proof techniques used for obtaining our results. 
...
The cost of a sequence of moves is the sum of the costs of the moves. Armed with these definitions, up to scaling, all the distances we can build from these depend solely upon the ratio of stamp cost to delay cost, giving: 
    ...
    \end{definition}
    \color{red}Note that we disallow setting the cost of either type of move as 0, as then the distance between any two words is always trivially zero.\color{black}
    ...
    \begin{example}\label{stamptoy}
        ...
        \color{red}This gives a cost 1 alignment, a stamp move of value 0.5 at the first position followed by a delay move of \color{blue}value -0.5 at the second.\color{black}
...
The best $d_0$ alignment for the example in the diagram below is $\gamma = (1,3,4)$ with minimum cost $d_0(\sigma, \gamma) = 4$, \color{red}with all events occurring as late as possible.\color{black}\
...
\color{red}We see here that the best aligning trace can be the same, but the total cost of alignment and method of computation are very different.\color{black}
\end{example}
...
\begin{proof}
  ...
  \color{red}We seek to prove that for all $\omega \in \mathcal{L}(N)$ i.e $\forall \omega : f_{\omega}(i) \in [a_i, b_i]$, $$d_N(\sigma, \gamma) \leq d_N(\sigma, \omega)$$\color{black}
...
\end{theorem}
\begin{proof}
	\color{red}From Theorem \ref{MixAlgoCorr} and Lemma \ref{doubleisdelay} we know that Algorithm \ref{MixAlignAlgo} computes the correct aligning trace for this precise class of distances, and the cost of the alignment can be obtained by computing the distance to the aligning trace. 
...
\color{blue}We chose the first six powers of ten as possible input trace lengths. 
...
\begin{tabular}{|c| c|}
    \hline
   \hspace*{0.5em} \color{red}Process Model Size\color{black} \hspace*{0.5em} & \hspace*{0.5em} Average Running Time (seconds)  \hspace*{0.5em}\\ 
    \hline
    10 & 0.00003\\
    100 & 0.00024\\
    1000 &0.00259\\
...
\color{red}Our main limitation is the fact that our methods only work on a very structurally restricted class of processes. 
As seen in this paper, even over sequential process models the problem is subtle, but in terms of practical application, the method needs to be generalised to be able to used for any process models that incorporate parallelism. 
