...
\usepackage{color}
\definecolor{firstrevision}{HTML}{000000}
\definecolor{secondrevision}{HTML}{ff0000}
...
%
\author{Neha Rino\inst{1,2}\orcidID{0009-0001-3273-1954} \and
...
A central notion in conformance checking is the notion of an alignment \cite{Adriansyah2014AligningOA}, or, the minimal series of edits, usually insertions or deletions, that transform an observed trace into a process trace.  
    ...
    We model event logs as a list of words over a finite alphabet, i.e., the set of possible discrete events. 
    The problem of calculating the alignment has been extensively studied \cite{Adriansyah2014AligningOA} \cite{BCC21}, usually using either Hamming distance or Levenshtein's edit distance. 
...
Distances over timed words have been explored before, such as in \cite{ABD18}, \cite{GHJ97} and \cite{CJM14}, which capture differences between the time-stamps attached to the same action label. 
...
As for process models, there are ways to use existing process mining notation to denote time constraints. BPMN 2.0 comes equipped with \emph{timer events}. 
    ...
    In particular, in this paper, we restrict ourselves to time Petri nets with \color{secondrevision}no branching transitions. \color{black}
...
These generalised weighted mixed distances allow users to decide what types of corrections in the observed behaviour should cost more to fix, as sometimes changing an event locally is the more viable option, while in other use cases, it might be more likely that shifts in local events necessarily cause consequent events to reflect the same shift. 
...
%   \subsection{Timed Alignments}
    ...
    \begin{example} Consider a model of composing and sending an email, where the intervals signify allowed durations between the current event and its immediate predecessor, depicted below with input and output places marked with an $i$ and an $o$: 
        ...
        \color{firstrevision}One process trace is $(\textrm{save draft}, 3)(\textrm{dispatch}, 5)(\textrm{sent}, 10)$, which depicts drafting the message in 3 units, then dispatching it in 2 units, and having it sent successfully.
        ...
        One possible optimally close process trace for this observation is $(\textrm{save draft}, 3)(\textrm{save draft},4)$ $(\textrm{dispatch}, 6)(\textrm{sent}, 11)$, which deletes the extra dispatch event and prepones the second draft saving event by a unit, allowing the rest of the trace to thereby be in time.\color{black}
    \end{example}
...
\color{firstrevision}It includes a more in depth discussion of the proof techniques used for obtaining our results. 
...
We believe these properties are very general, and with the appropriate extension will assist in solving the purely timed alignment problem for a wider structural class of models, and so a deeper discussion about them is valuable.\color{black}
...
The cost of a sequence of moves is the sum of the costs of the moves. Armed with these definitions, up to scaling, all the distances we can build from these depend solely upon the ratio of stamp cost to delay cost, giving: 
    ...
    \end{definition}
    \color{firstrevision}Note that we disallow setting the cost of either type of move as 0, as then the distance between any two words is always trivially zero.\color{black}
    ...
    \begin{example}\label{stamptoy}
        ...
        \color{firstrevision}This gives a cost 1 alignment, a stamp move of value 0.5 at the first position followed by a delay move of \color{secondrevision}value -0.5 at the second.\color{black}
...
\color{firstrevision}
\begin{lemma}\label{MixAlgoCorr}Given two time sequences $\sigma = (\sigma_1, \dots, \sigma_n)$ and $\gamma = (\gamma_1, \dots, \gamma_n)$ with sequential underlying processes,
...
To this end, we begin by defining the type of process model we will be working with.\color{black}
...
The best $d_0$ alignment for the example in the diagram below is $\gamma = (1,3,4)$ with minimum cost $d_0(\sigma, \gamma) = 4$, \color{firstrevision}with all events occurring as late as possible.\color{black}\
...
\color{firstrevision}We see here that the best aligning trace can be the same, but the total cost of alignment and method of computation are very different.\color{black}
\end{example}
...
\begin{proof}
  ...
  \color{firstrevision}We seek to prove that for all $\omega \in \mathcal{L}(N)$ i.e $\forall \omega : f_{\omega}(i) \in [a_i, b_i]$, $$d_N(\sigma, \gamma) \leq d_N(\sigma, \omega)$$\color{black}
...
\end{theorem}
\begin{proof}
	\color{firstrevision}From Theorem \ref{MixAlgoCorr} and Lemma \ref{doubleisdelay} we know that Algorithm \ref{MixAlignAlgo} computes the correct aligning trace for this precise class of distances, and the cost of the alignment can be obtained by computing the distance to the aligning trace. 
    Since both these tasks have linear runtime complexity, overall purely timed alignment for this class of distances, to sequential process models, has been proven to be computable in $\mathcal{O}(|N|)$ time.\color{black}
	\end{proof}

\section{Implementation}
\color{firstrevision}
As shown in the previous sections, both distance computation and alignment tasks for a large class of mixed distances is efficiently computable, in fact the provided algorithms have been proven to run in time directly proportional to the size of the model alone.\color{black}
...
\color{secondrevision}We chose the first six powers of ten as possible input trace lengths. 
Then, using Python's random integer generator we ran the alignment algorithm and distance calculation algorithm on 1,000,000 different randomly generated traces, wherein each gap between events was a random integer between 0 and 1,000,000. 
We then averaged the running time for each input length, and report the same here. 
\color{black}These tests allow us to empirically confirm the linear relationship between model size and running time, as can be clearly seen in the run-time table below. 
These running times were obtained on an Apple M1 chip with 8 cores, running at 3.2 GHz, 8GB RAM machine.
\centerline{
\begin{tabular}{|c| c|}
    \hline
   \hspace*{0.5em} \color{firstrevision}Process Model Size\color{black} \hspace*{0.5em} & \hspace*{0.5em} Average Running Time (seconds)  \hspace*{0.5em}\\ 
    \hline
    10 & 0.00003\\
    100 & 0.00024\\
    1000 &0.00259\\
...
We demonstrate that computing these distances is not straightforward even when we restrict our process models to have no parallelism, i.e. the time constraint on each event only relates it to a single predecessor event at most. 
...
\color{firstrevision}Our main limitation is the fact that our methods only work on a very structurally restricted class of processes. 
As seen in this paper, even over sequential process models the problem is subtle, but in terms of practical application, the method needs to be generalised to be able to handle any process models that incorporate parallelism. 
While considering models that branch outwards might retain our method's efficiency, we suspect models with synchronisation between different events in particular would be tougher to tackle, due to the fact that mixed moves will cease to commute with each other chronologically. 
A perspective in this direction would be to encode the purely timed alignment problem as an ILP problem, and use generic ILP solvers to find alignments for general time Petri net process models.\color{black}
...
The notions of stamp and delay we employ implicitly assume that events as represented in the process model only share resources as implied by token-flow, and more complex dependencies will render several move sequences impossible to realise in the real world. \color{secondrevision}Also in an organizational perspective, many complex reasons can explain why a task is delayed, which could be expressed only in richer formalisms.\color{black}
