...
%	\subsection{Timed Alignments}
	...
	\begin{example} Consider a model of composing and sending an email, where the intervals signify allowed durations between the current event and its immediate predecessor, depicted below with input and output places marked with an $i$ and an $o$: 
		...
		\[
			\begin{tikzcd}
				\eye_i & {\overset{[1,1]}{\fbox{\textrm{dispatch}}}} & \bigcirc & {\overset{[5, 5]}{\fbox{\textrm{sent}}}} & \bigcirc_o \\
				{\underset{[1, \infty]}{\fbox{\textrm{draft}}}} & {\underset{[0,5]}{\fbox{\textrm{unsend}}}} & {}
		...
		One process trace is $(\textrm{draft}, 5)(\textrm{dispatch}, 6)(\textrm{sent}, 11)$, which depicts drafting the message in 5 units and then dispatching it, and having it send successfully. An example of an observed trace which does not conform to the process above would be $(\textrm{draft}, 3)(\textrm{draft}, 5)(\textrm{dispatch}, 7)(\textrm{dispatch}, 7)(\textrm{sent}, 12)$. 
		Clearly, there is an extraneous letter here, as the same message cannot be dispatched twice, and the timestamps for dispatch and sending come too late. 
		One possible optimally close process trace for this observation is $(\textrm{draft}, 3)(\textrm{draft},6)(\textrm{dispatch}, 7)(\textrm{sent}, 12)$, which deletes the extra dispatch event, and extends the second draft event by a unit, allowing the rest of the trace to thereby be in time. 
	\end{example}
...
The untimed part is already aligned, and in order to make the timestamps fit we can edit the timestamp of $d$ to give the following process trace $(v_1, \gamma_1) = (d, 3)(e, 6)(f, 6)$. 
	...
	We present linear time algorithms for both computing $d_1$ and solving the alignment problem in this setting. 
	The full proofs for all of the results stated here can be found in \cite{arxivmix}. 
...
The cost of a sequence of moves is the sum of the costs of the moves. Armed with these definitions, upto scaling, all the distances we can build from these depend upon the ratio of stamp cost to delay cost, giving rise to a general class of weighted mixed distances. 
	\begin{definition}[Weighted mixed distance: $d_{\alpha}$]
		We define the cost of a stamp move per unit displacement in a weighted mixed distance to be $c_s$ and the cost of the delay move per unit displacement to be $c_d$, and assume that we always have $c_d \cdot c_s \neq 0, c_s, c_d \in \mathbb{R}^+ \cup \{\infty\}$. Given any two timing functions $\tau_1, \tau_2$ over the same causal process $(CN, p)$ and $\alpha \in \mathbb{R}^+ \cup \{\infty\}$, we define the $\alpha$ weighted mixed distance $d_{\alpha}$ as follows 
